...
//	int size = 0; // 배열에 저장된 객체의 개수
//	int capacity = 0; // 배열의 크기
//
////	2. MyVector클래스에 생성자 MyVector(int capacity)와 
////	기본생성자(capacity=16)를 추가하시오.
//	MyVector(int capacity) {
////		this.capacity = capacity; // 필요없음
...
//	5.
	Object get(int index) {
		if (!(0 <= index && index < size)) {
			throw new ArrayIndexOutOfBoundsException("index의 범위가 유효하지 않습니다." + index);
		}
		return objArr[index];
	}
